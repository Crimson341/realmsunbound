                ...
                <div className="flex flex-col md:flex-row items-center justify-between mb-12 gap-6">
                    ...
                    <div className="flex flex-wrap items-center gap-4 w-full md:w-auto font-sans">
                        <div className="relative group flex-1 md:w-64">
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-500 group-focus-within:text-purple-400 transition-colors" size={18} />
                            <input 
                                type="text" 
                                placeholder="Search incantations..." 
                                value={search}
                                onChange={(e) => setSearch(e.target.value)}
                                className="w-full bg-stone-900/50 border border-stone-800 rounded-full py-2 pl-10 pr-4 text-sm focus:outline-none focus:border-purple-500 transition-all placeholder:text-stone-600"
                            />
                        </div>
                        <div className="relative">
                            ...
                            <select 
                                ...
                                className="bg-stone-900/50 border border-stone-800 rounded-full py-2 pl-10 pr-8 text-sm focus:outline-none focus:border-purple-500 appearance-none cursor-pointer hover:bg-stone-900 transition-colors"
                            >
...
const SpellCard = ({ spell }: { spell: any }) => {
    ...
    return (
        <div className="group relative bg-stone-900 border border-stone-800 rounded-xl p-1 hover:border-purple-500/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_10px_30px_-10px_rgba(168,85,247,0.2)] h-full flex flex-col">
            {/* Card Content */}
            <div className="bg-stone-950 rounded-lg p-5 h-full flex flex-col relative overflow-hidden">
                {/* Decorative Corner */}
                ...
                <div className="flex items-start justify-between mb-4 relative z-10">
                    <div className="p-3 bg-stone-900 rounded-lg border border-stone-800 group-hover:border-purple-500/30 transition-colors">
                        {getIcon(spell.school)}
                    </div>
                    <div className="text-xs font-bold text-stone-500 uppercase tracking-wider border border-stone-800 px-2 py-1 rounded bg-stone-900">
                        Lvl {spell.level}
                    </div>
                </div>

                <h3 className="text-xl font-bold text-white mb-1 font-serif group-hover:text-purple-400 transition-colors">{spell.name}</h3>
                <div className="text-xs font-bold text-purple-400 mb-4 uppercase tracking-wide">{spell.school}</div>

                <div className="grid grid-cols-2 gap-2 mb-4 text-xs font-sans text-stone-500">
                    <div className="bg-stone-900/50 p-2 rounded border border-stone-800/50">
                        <span className="block text-stone-600 uppercase text-[10px]">Time</span>
                        {spell.castingTime}
                    </div>
                    <div className="bg-stone-900/50 p-2 rounded border border-stone-800/50">
                        <span className="block text-stone-600 uppercase text-[10px]">Range</span>
                        {spell.range}
                    </div>
                    <div className="bg-stone-900/50 p-2 rounded border border-stone-800/50">
                        <span className="block text-stone-600 uppercase text-[10px]">Duration</span>
                        {spell.duration}
                    </div>
                    <div className="bg-stone-900/50 p-2 rounded border border-stone-800/50">
                        <span className="block text-stone-600 uppercase text-[10px]">Comp</span>
                        {spell.components || 'V, S'}
                    </div>
                </div>

                <p className="text-sm text-stone-400 font-sans leading-relaxed line-clamp-4 mb-4 flex-grow">
                    {spell.description}
                </p>

                {/* Footer Stats */}
                {(spell.damageDice || spell.save) && (
                    <div className="mt-auto pt-4 border-t border-stone-900 flex items-center justify-between text-xs font-sans font-bold">
                        {spell.damageDice && (
